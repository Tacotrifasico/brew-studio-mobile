...
class ForkTechniqueShareUseCase(
    ...
    suspend operator fun invoke(shareId: String): Result<PreparationTechnique> {
        ...
        val forkedName = "${sourceTech.name} · fork"

        val forkedTechnique = sourceTech.copy(
            id = newTechId,
        ...
        techniqueRepository.save(forkedTechnique)
        socialRepository.forkShare(shareId, currentUser.id)

        return Result.success(forkedTechnique)
    }
}
