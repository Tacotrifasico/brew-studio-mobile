...
## D-052 — El checkpoint precede al barrido remoto

Guardar la hora local al terminar catorce consultas abre una carrera: una escritura remota posterior a la consulta de su tabla, pero anterior a esa hora final, no aparece en el barrido actual ni en el siguiente. El coordinador conserva ahora la fecha HTTP del primer response menos cinco minutos como frontera segura. Todo cambio concurrente vuelve a consultarse gracias al límite inclusivo, aunque otro ya se haya descargado; el resolver por UUID, propietario, `updated_at` y versión hace la repetición idempotente. Si el proxy omite `Date`, la frontera es el inicio local con el mismo solapamiento. Se acepta transferencia adicional para privilegiar integridad y tolerar latencia o desfase moderado de reloj.

## D-053 — Ningún push puede degradar una fila remota

Enviar primero con `resolution=merge-duplicates` permitía que una edición local atrasada reemplazara una fila remota más nueva antes de que el pull pudiera compararlas. iOS aplica ahora una escritura condicional: `PATCH` sólo cuando la fecha remota no supera la local, seguido de una inserción que ignora UUID duplicados. Si ambas operaciones no modifican filas, recupera el registro exacto y sólo retira la outbox cuando el resolver adopta el remoto o confirma identidad; una respuesta contradictoria conserva el cambio con backoff. El perfil usa el mismo camino. La outbox renombra su campo reservado `entityName` a `tableName` mediante identificador de migración y repara valores heredados vacíos por UUID/tipo antes de compactar, evitando abandonar cambios offline creados por builds anteriores.

## D-054 — La cuota de IA es atómica y el contenido no se registra

Contar solicitudes y después insertar permite que llamadas paralelas observen el mismo total y excedan el límite. Una RPC `SECURITY INVOKER` toma un bloqueo transaccional derivado del usuario autenticado, decide la ventana de cinco por minuto e inserta la reserva sin separar ambas operaciones. La función Edge registra después sólo un código cerrado de resultado; no conserva entrada, prompt renderizado ni salida. El contrato v2 valida en Swift y servidor todos los límites, separa el `systemInstruction` de las mediciones JSON no confiables, rechaza controles y respuestas mayores a 90 palabras y solicita `store: false`. Gemini nunca modifica datos y cualquier fallo vuelve al motor local.
