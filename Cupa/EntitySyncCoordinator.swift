...
enum RemoteFieldKind { case string, integer, double, boolean, uuid, date, dateOnly, json }
struct RemoteField { let local: String; let remote: String; let kind: RemoteFieldKind }
struct SyncEntityDescriptor { let entityName: String; let table: String; let fields: [RemoteField]; var profileIdentity = false }

enum CoreSyncSchema {
    static let descriptors: [SyncEntityDescriptor] = [
        .init(entityName: "CoffeeBeanRecord", table: "coffee_beans", fields: [f("name"),f("brand"),f("origin"),f("producer"),f("variety"),f("process"),f("altitudeMetersValue","altitude_meters",.integer),f("roastLevel","roast_level"),f("roastDate","roast_date",.dateOnly),f("openedDate","opened_date",.dateOnly),f("initialQuantityGrams","initial_quantity_grams",.double),f("remainingQuantityGrams","remaining_quantity_grams",.double),f("notes")]),
        .init(entityName: "GrinderRecord", table: "grinders", fields: [f("name"),f("brand"),f("model"),f("grinderType","grinder_type"),f("scaleUnit","scale_unit"),f("minimumSetting","minimum_setting",.integer),f("maximumSetting","maximum_setting",.integer),f("calibrationNotes","calibration_notes"),f("notes")]),
        .init(entityName: "EquipmentRecord", table: "equipment", fields: [f("equipmentType","equipment_type"),f("name"),f("brand"),f("model"),f("capacityMlValue","capacity_ml",.integer),f("configuration"),f("notes"),f("isFavorite","is_favorite",.boolean),f("isActive","is_active",.boolean)]),
        .init(entityName: "RecipeRecord", table: "recipes", fields: [f("name"),f("recipeKind","recipe_kind"),f("intention"),f("suggestedMethodId","suggested_method_id",.uuid),f("suggestedMethodName","suggested_method_name"),f("isFavorite","is_favorite",.boolean),f("tags"),f("visibility"),f("originalEntityId","original_entity_id",.uuid),f("rootEntityId","root_entity_id",.uuid),f("copyMode","copy_mode")]),
        ...
        .init(entityName: "TechniqueRecord", table: "techniques", fields: [f("name"),f("methodId","method_id",.uuid),f("methodName","method_name"),f("recipeId","recipe_id",.uuid),f("beanId","bean_id",.uuid),f("grinderId","grinder_id",.uuid),f("doseGrams","dose_grams",.double),f("waterMl","water_ml",.integer),f("ratio",kind:.double),f("temperatureC","temperature_c",.integer),f("executionMode","execution_mode"),f("grindValue","grind_value",.double),f("grindDescription","grind_description"),f("grindUnit","grind_unit"),f("notes"),f("techniqueDescription","description"),f("totalTimeSeconds","total_time_seconds",.integer),f("visibility"),f("originalEntityId","original_entity_id",.uuid),f("rootEntityId","root_entity_id",.uuid),f("copyMode","copy_mode")]),
        .init(entityName: "TechniqueStepRecord", table: "technique_steps", fields: [f("techniqueId","technique_id",.uuid),f("stepNumber","step_number",.integer),f("title"),f("durationSeconds","duration_seconds",.integer),f("waterAddedMl","water_added_ml",.integer),f("waterAccumulatedMl","water_accumulated_ml",.integer),f("intensity"),f("gesture"),f("stepNote","step_note"),f("coverageValue","coverage",.double),f("flowValue","flow",.double),f("secondaryAction","secondary_action")]),
        .init(entityName: "BrewSessionRecord", table: "brew_sessions", fields: [f("techniqueId","technique_id",.uuid),f("recipeId","recipe_id",.uuid),f("beanId","bean_id",.uuid),f("grinderId","grinder_id",.uuid),f("techniqueNameSnapshot","technique_name_snapshot"),f("methodNameSnapshot","method_name_snapshot"),f("beanNameSnapshot","bean_name_snapshot"),f("grinderNameSnapshot","grinder_name_snapshot"),f("doseGrams","dose_grams",.double),f("waterMl","water_ml",.integer),f("ratio",kind:.double),f("temperatureC","temperature_c",.integer),f("grindDescription","grind_description"),f("elapsedSeconds","elapsed_seconds",.integer),f("completedAt","completed_at",.date),f("stepsSnapshotJSON","steps_snapshot",.json)]),
        ...
        .init(entityName: "CupSessionRecord", table: "cup_sessions", fields: [f("brewSessionId","brew_session_id",.uuid),f("tastingId","tasting_id",.uuid),f("techniqueNameSnapshot","technique_name_snapshot"),f("beanNameSnapshot","bean_name_snapshot"),f("rating",kind:.double)]),
        .init(entityName: "LabExperimentRecord", table: "lab_experiments", fields: [f("method"),f("coffeeGrams","coffee_grams",.double),f("waterMl","water_ml",.integer),f("ratio",kind:.double),f("temperatureC","temperature_c",.integer),f("grindClicks","grind_clicks",.integer),f("freshness"),f("timeSeconds","time_seconds",.integer),f("altitudeMeters","altitude_meters",.integer),f("cityName","city_name"),f("notes"),f("extractionIndex","extraction_index",.double),f("summary")]),
        .init(entityName: "UserProfileRecord", table: "profiles", fields: [f("displayName","display_name"),f("alias"),f("biography"),f("avatarColor","avatar_color"),f("favoriteMethods","favorite_methods"),f("isPrivate","is_private",.boolean)], profileIdentity: true)
    ]
...
final class EntitySyncCoordinator: ObservableObject {
    ...
    func encode(_ record: NSManagedObject, descriptor: SyncEntityDescriptor, ownerId: UUID) throws -> Data {
        ...
        if !descriptor.profileIdentity { row["owner_id"] = ownerId.uuidString }
    ...
    func merge(_ row: [String: Any], descriptor: SyncEntityDescriptor, expectedOwner: UUID) throws {
        ...
        let remoteOwner = descriptor.profileIdentity ? id : (row["owner_id"] as? String).flatMap(UUID.init(uuidString:))
