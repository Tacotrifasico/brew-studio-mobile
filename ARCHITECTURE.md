...
Las descargas se leen completas antes de tocar el contexto visible y se aplican en una sola transacción local. Si falla una tabla o una técnica llega sin todos sus pasos, con acumulados incoherentes, proporción incorrecta o duración distinta de la suma de pasos, el contexto revierte todo el lote y conserva el agregado anterior; el checkpoint tampoco avanza.
...
La Edge Function de Gemini autentica primero el JWT y reserva cuota mediante una RPC transaccional serializada por usuario; no realiza el patrón vulnerable contar→insertar. El prompt fijo viaja como `systemInstruction` y el JSON del usuario como contenido no confiable separado. Entrada y salida comparten límites equivalentes con Swift, la salida no puede superar 90 palabras y sólo se persisten `prompt_version` y `outcome_code`, nunca las mediciones ni la respuesta.
